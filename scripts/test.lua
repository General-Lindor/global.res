...
function string:levenshteinWithClear(otherString)
    ...
    do
        store = function(i, j, value)
            ...
            if ((has(i - 1, j)) and (has(i, j - 1))) then
                do clear(i - 1, j - 1) end
            end
            if ((has(i + 1, j)) and (has(i + 1, j - 1))) then
                do clear(i, j - 1) end
            end
            if ((has(i, j + 1)) and (has(i - 1, j + 1))) then
                do clear(i - 1, j) end
            end
        end
    end
...
--returns the count of difference letters between two strings as an integer value
function string:levenshteinWrong(otherString)
...
local ti = os.clock()
do print(a:levenshteinWithClear(b)) end
do print(os.clock() - ti) end
do ti = os.clock() end
...
do print(os.clock() - ti) end
do ti = os.clock() end
do print(c:levenshteinWithClear(d)) end
do print(os.clock() - ti) end
print("Done Comparing")
do c = c:prepare() end
do d = d:prepare() end
